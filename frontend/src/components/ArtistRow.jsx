...
export default function ArtistRow({ rank, artist, metric, index = 0 }) {
  return (
    <motion.div
      ...
      className="group flex items-center gap-2 sm:gap-4 rounded-xl px-2 sm:px-4 py-2.5 sm:py-3 hover:bg-fg/5 transition-colors"
    >
      ...
      <div className="min-w-0 flex-1">
        <p className="truncate text-sm sm:text-base font-medium text-fg">{artist.artist_name}</p>
        {artist.track_count != null && (
          <p className="truncate text-xs sm:text-sm text-mist">{artist.track_count} tracks · combined</p>
        )}
      </div>

      <span className="font-mono tabular text-xs sm:text-sm text-mist group-hover:text-fg transition-colors whitespace-nowrap flex-shrink-0">
        {metric}
      </span>
    </motion.div>
  );
